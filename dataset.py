...
def load_train_label(train_path, label_file, image_size, classes):
    ...
    for fl in files:
        file_name = fl.split("/")[-1].replace(".jpeg","")
...
def load_numpy_data():
    images = np.load("train_256.npy") #"2block_256.npy")
    labels = np.load("label_1-6.npy") #"2block_Label.npy")
    ids = np.load("fnames_1-6.npy") #"2block_fnames.npy")
    cls = np.load("class_1-6.npy") #"2block_classes.npy")
...
def read_train_sets_with_labels(train_path,label_file ,image_size, classes, validation_size=0):
    class DataSets(object):
        pass
    data_sets = DataSets()

    images, labels, ids, cls = load_numpy_data()#load_train_label(train_path, label_file,image_size, classes)
    images, labels, ids, cls = shuffle(images, labels, ids, cls)  # shuffle the data
